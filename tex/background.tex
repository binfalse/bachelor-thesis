...
\section{Using graph databases to store \sysbio models}
	...
	But since architectural planing and design is an essential part of software development, it is necessary to define formal data structure and relations.
	
	Even though \neoj, introduced in section \ref{sec:graph-db}, is part of the schema-free noSQL, it provides mechanisms to work with schema and also enforce them\footnote{\url{https://neo4j.com/docs/developer-manual/current/cypher\#cypher-schema}}.
	Unfortunately the neo4j team does not use any establish modeling language, but relies on example graphs for their documentation and examples.
	...
	However, as \cite{Siriwaradhana2014} shows it is fairly easy to translate standard ER-models to a graph-database structure. An example for a translation can be seen in figure \ref{fig:example-er-diagram}.
	\todo{check usage of cite style}
	...
	In case of a simple binary relation, a edge with type (\neoj label) of the relation name is added. Similar to entities the attributes are becoming properties. end- and start-point fo the edge are the corresponding vertex-types to the two entities connected by the relation.
	
	N-ary relation on the other hand cannot be easily translated. To map those it is necessary to add a vertex with the name of the relation, due to the restriction of property graphs regarding relations. This new vertex is representing the relation and is assigned all associated properties. The participating entities in this relation are connected via edges, which are labeled after the role of the participating entity or if no role is specified the edge name could be a combination of relationship- and target-vertex name.
